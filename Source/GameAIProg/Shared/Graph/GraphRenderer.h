...
namespace GameAI
{
    ...
    private:
        ...
        void DrawConnection(Graph const& Graph, ConnectionType const& Connection,
            FColor Color = Graphs::DefaultConnectionDrawColor) const;
    };
    ...
    template <typename ConnectionType>
        requires GameAI::is_connection_type<ConnectionType>
    void GraphRenderer::DrawConnection(Graph const& Graph, ConnectionType const& Connection, FColor Color) const
    {
        if (!World)
            return;
